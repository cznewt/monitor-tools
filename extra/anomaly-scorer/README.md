...
Two training-free siblings score the same query → score → export shape without
Prophet: [`anomaly-scorer-iqr`](../anomaly-scorer-iqr/) (interquartile-range
fence) and [`anomaly-scorer-zscore`](../anomaly-scorer-zscore/) (robust median +
MAD z-score). This Prophet build is the one to reach for when the metric has real
trend or daily/weekly seasonality; the siblings are lighter and better on
flat-ish / noisy signals.
